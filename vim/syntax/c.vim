...
" highlight struct names (does not work right now)
...
syntax match  cDefineName "^#define\s\+\h\w\+("he=e-1 contains=ALLBUT,@cParenGroup
syntax match  cDefineName "^#define\s\+\h\w\+\s"he=e-1 contains=cDef

" highlight typedef names
syntax match cTagList "^}\(\s*\*\?\h\w*\s*,\?\)\+.*;$"
"syntax match cTagName "\h\w*\s*[,;]"he=e-1 containedin=cTagList contained
...
syntax match cFuncName "\w+"
syntax region cFuncDef oneline start="^\h" matchgroup=cFuncName end="(.*)"rs=s-1 contains=cType
...
hi def link cEmptyDefine	Macro
...
hi def link cTagName            Function
hi def link cFuncProto          Defined
hi def link cDefineName         Function
hi def link cFuncLine           Function
hi def link cFuncDef            Function
hi def link cDef		Macro
